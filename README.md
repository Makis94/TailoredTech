...
See [`SCOPE.md`](SCOPE.md) for the division of responsibility between human direction and AI-assisted implementation. In short: this project was built with Claude Code (Anthropic's CLI coding agent) — the NestJS API, the React frontend, the Prisma schema, and this README were implemented by directing Claude Code, with the output reviewed and corrected along the way rather than accepted blindly.
